...
describe 'base_windows::system_metrics' do
  ...
  context 'create the consul-template files for telegraf' do
    ...
    telegraf_metrics_config_content = <<~CONF
      # Telegraf Configuration
      ...
      {{ end }}
    CONF
    it 'creates telegraf metrics template file in the consul-template template directory' do
      expect(chef_run).to create_file('c:/config/consul-template/templates/telegraf_metrics.ctmpl')
        .with_content(telegraf_metrics_config_content)
    end

    consul_template_telegraf_metrics_content = <<~CONF
      # This block defines the configuration for a template. Unlike other blocks,
      # this block may be specified multiple times to configure multiple templates.
      # It is also possible to configure templates via the CLI directly.
      template {
        ...
        # called the "Consul Template template". This option is required if not using
        # the `contents` option.
        source = "c:/config/consul-template/templates/telegraf_metrics.ctmpl"
    ...
    CONF
    it 'creates telegraf_metrics.hcl in the consul-template template directory' do
      expect(chef_run).to create_file('c:/config/consul-template/config/telegraf_metrics.hcl')
        .with_content(consul_template_telegraf_metrics_content)
    end
  end
end
